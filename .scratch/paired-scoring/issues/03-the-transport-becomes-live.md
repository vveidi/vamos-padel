...
the phone" and stop taking taps (ticket 05), and the scoreboard has to say the
watch is gone. Both read it here.

**The mirrored session's own channel is not used** — the spec says why, under
"The mirrored workout keeps the phone alive", and names it as the fallback if
`sendMessage` misbehaves on a live pair.
