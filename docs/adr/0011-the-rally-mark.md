...
It fires on the journal coming back and never on the tap, which is ADR-0009's rule and not an exception to it: the watch draws nothing it has not been given. The watch knows which returning rally is its own from the intent it is still waiting on.
...
- **A refused intent buzzes and does not light.** The haptic is optimistic and fires with the finger; the mark waits for the journal. Where they disagree the mark is the honest one, and `watch-tap-mode` 02 has already accepted the other half of that trade.
