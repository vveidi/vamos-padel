    ...
    static var migrator: DatabaseMigrator {
        ...
        migrator.registerMigration("v1") { db in
            try db.create(table: "match") { t in
                ...
                // расписка очереди: часы остаются источником правды до
                // подтверждения (ADR-0002) и потому обязаны помнить, что
                // доехало, а что нет. На телефоне колонка есть и всегда
                // пуста — доставлять ему некуда.
                //
